...
Превращает транскрипт рабочего созвона в готовые задачи для трекера **YouGile**: с названием,
описанием, исполнителем, контролёром, сроком и приоритетом — после ревью менеджером.
...
**Загрузка** — файл или ссылка (Яндекс.Диск / Google Drive), выбор проекта из YouGile,
база команды:
...
валидатора; по клику виден фрагмент-источник из транскрипта; правка inline; одобрение на каждой:
...
```
Аудио/транскрипт
  → транскрипция (Whisper)            transcribe.py
...
cp .env.example .env          # вписать ключи Groq/OpenRouter и YouGile
```
...
- `.env` (ключи Groq/OpenRouter/YouGile) и приватные транскрипты (`raw/`) **исключены** из
  репозитория через `.gitignore`. Для настройки используйте `.env.example`.
